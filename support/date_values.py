...
class DateFormat(Enum):
    ...
    yyyyMMdd = "%Y%m%d"
    yyyyMM = "%y%m"
    foramttedYyyyMMddHHmmss = "%y-%m-%d %H:%M:%S"
...
class DateValues:
    ...
    def get_before_one_month(base_day: str = None):
        ...
        if base_day:
            ...
            return (datetime.strptime((base_day[:6] + "01"), DateFormat.yyyyMMdd.value) - relativedelta(months=1)).strftime(DateFormat.yyyyMM.value)
    ...
    def get_date_list(start_day, end_day):
        """
        Return date list betwwen start and end day
        ...
        >>> result = date_value.get_date_list(start_day='20240401', end_day='20240409')
        >>> result
        ...
        try:
            ...
            for i in range((end_datetime - start_datetime).days + 1):
                datelist.append(datetime.strptime(start_datetime, DateFormat.yyyyMMdd) +timedelta(i).strftime(DateFormat.yyyyMMdd.value))
            return datelist
        except ValueError:
